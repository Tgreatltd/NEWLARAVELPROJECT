<div class="container mx-auto mt-8">
    <h1 class="text-blue-700">NEWS ADMIN PAGE</h1>
    <div>
...
@endif
        <form wire:submit.prevent="update"  enctype="multipart/form-data">
           ...
           <div class="mb-4">
            ...
            <input wire:model="title" type="text" name="title" id="title" class="w-full py-2 px-3 border rounded"
             placeholder="Enter a title" value="{{$newsId->title}}">
            @error('title')
        ...
        <div class="mb-4">
            ...
            placeholder="Enter description" value="{{$newsId->description}}">
            @error('description')
            <small class="text-red-800">{{$message}}</small>
        @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea wire:model="storyline" name="storyline" class="w-full py-2 px-3 border rounded" rows="8"
             placeholder="Enter storyline">{{$newsId->storyline}}</textarea>
            @error('storyline')
            <small class="text-red-800">{{$message}}</small> 
         @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <input wire:model="image" type="file" name="image" id="image" class="w-full py-2 px-3 border rounded bg-red-800"
             accept="image/*" value="{{$newsId->image}}">
        @error('image')
